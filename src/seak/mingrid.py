"""Brent’s method (a 1D numerical optimization algorithm) from FaST-LMM.
...
def minimize1D(f, evalgrid=None, nGrid=10, minval=0.0, maxval=0.99999, verbose=False, brent=True, check_boundaries=True,
    ...
    if brent:  # do Brent search in addition to rest?
        ...
        onebrent = False
        for i in range(resultgrid.shape[
                            0] - 2):  # if any triplet is found, where the inner point is a local optimum expand search
